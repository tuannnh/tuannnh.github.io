...
function truncateBetween(val, min, max) {
...
function signedDistanceToCircle([px, py], { center: [cx, cy], radius }) {
  return getDistanceBetweenPoints([cx, cy], [px, py]) - radius;
}

function getSegmentVector([x1, y1], [x2, y2]) {
  return [x2 - x1, y2 - y1];
}

function getDistanceBetweenPointRect(p, r) {
